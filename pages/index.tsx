  ...
  return (
    <>
      ...
      <main className='relative snap-y snap-mandatory scroll-smooth h-screen w-full overflow-y-scroll'>
          ...
          <InView triggerOnce={false} threshold={0.01} initialInView={true} fallbackInView={true} onChange={(inView) => sethomeInView(!homeInView)}>
          {({ inView, ref, entry }) => (
            <section ref={ref} id="home" className="snap-start bg-lavaOrange w-full h-full flex flex-col gap-4 justify-center overflow-hidden p-4 md:gap-6">
              <h1 className="animate-fadeInDown animate-fast text-violet-500 uppercase font-extrabold text-center drop-shadow-lg text-5xl md:text-8xl lg:9xl ">Colorful Code</h1>
              <h2 className="animate-fadeIn animate-slow animate-delay-[600ms] text-zinc-800 lowercase font-light text-center text-sm sm:text-lg sm:font-normal">Bringing a human touch to technology</h2>
              <a className='animate-fadeInUpBig animate-duration-[2.1s] animate-delay-[600ms] self-center focus:ring-1 outline-none' href="#about"><FaChevronDown className='fill-black hover:fill-red-500 active:fill-red-700 animate-shakeY animate-infinite animate-duration-[10s] self-center text-4xl landscape:md:text-4xl md:text-7xl lg:text-6xl'/></a>
            </section>
          )}
          </InView>
        ...
        {({ inView, ref, entry }) => (
          <section id="about" ref={ref} className='snap-start bg-amber-400 w-full h-full flex flex-col overflow-y-auto p-8'>
            <div className={`relative flex justify-center mb-4 ${inView ? 'animate-slideInDown animate-fast' : ''}`}>
              ...
              <h1 className='text-shadow-sm relative uppercase text-white text-center font-semibold text-3xl mb-2 landscape:md:text-4xl md:text-7xl lg:text-9xl lg:mb-10 lg:text-shadow-lg'>About</h1>
            </div>
            <div className="flex flex-col align-middle justify-center lg:flex-row lg:gap-10">
              ...
              <h2 className={`text-center font-light text-xl text-md md:text-2xl md:font-light lg:text-4xl lg:mb-2 lg:font-light ${inView ? 'animate-zoomIn animate-faster' : ''}`}>Tools I use</h2>
              <div className="flex justify-center">
                <div className={`flex flex-wrap max-w-3xl p-2 justify-center gap-0 md:gap-x-10 gap-y-4 ${inView ? 'animate-zoomIn animate-faster' : ''}`}>
        ...
        <InView triggerOnce={true} threshold={0.01} initialInView={false} fallbackInView={true}>
        {({ inView, ref, entry }) => (
          <section id="projects" ref={ref} className='snap-start bg-teal-500  w-full h-full flex flex-col overflow-y-auto p-8'>
            <div className={`relative flex justify-center mb-4 ${inView ? 'animate-slideInLeft animate-faster sm:animate-slideInDown sm:animate-fast' : ''}`}>
              <Brushstroke className="absolute w-1/2 h-1/2 top-1/2 fill-green-300"/>
              <h1 className='text-shadow-sm relative uppercase text-white text-center font-semibold text-3xl mb-2 landscape:md:text-4xl md:text-7xl lg:text-9xl lg:mb-10 lg:text-shadow-lg'>Projects</h1>
            </div>
            <p className={`text-xs font-thin self-center mb-4 md:text-base ${inView ? 'animate-slideInRight animate-faster sm:animate-slideInUp' : ''}`}> Most of my work are things I can not share here due to client confidentiality. However, I have more personal projects that will be added soon.</p>
            <div className={`flex flex-wrap grow gap-y-10 gap-x-20 justify-center overflow-y-auto ${inView ? 'animate-slideInLeft animate-faster sm:animate-slideInUp sm:animate-duration-500' : ''}`}>
        ...
        <InView triggerOnce={true} threshold={0.01} initialInView={false} fallbackInView={true}>
        {({ inView, ref, entry }) => (
          <section id="contact"  ref={ref} className='snap-start bg-indigo-500 w-full h-full flex flex-col align-middle overflow-y-auto p-8'>
            <div className={`relative flex justify-center mb-4 ${inView ? 'animate-zoomIn animate-fast' : ''}`}>
                ...
                <h1 className='text-shadow-sm relative uppercase text-white text-center font-semibold text-3xl mb-2 landscape:md:text-4xl md:text-7xl lg:text-9xl lg:mb-10 lg:text-shadow-lg'>Contact</h1>
              </div>

            {/* Main container in this section */}
            <div className={`flex flex-wrap self-center space-y-4 w-full shadow-2xl rounded-lg border border-slate-600 sm:w-fit p-2 ${inView ? 'animate-slideInUp animate-faster' : ''}`}>
              <div className={`flex w-full justify-center mt-4 ${inView ? 'animate-flipInX animate-delay-[400ms]' : ''}`}>
                <h2 className="text-center text-xs font-bold shadow-lg border rounded-lg border-black bg-gradient-to-r from-lime-200 to-emerald-200 p-2">Currently available for new contracts.</h2>
              </div>
            ...
            <div className="w-full mt-10 mb-6 flex justify-center">
              <a href="#home" className="shadow-md border border-indigo-600 bg-indigo-200 max-w-fit flex justify-center align-middle rounded-full px-2 py-3 animate-twBounce animate-infinite focus:outline-none focus:ring-2 focus:ring-slate-300 focus:ring-offset-2 ring-offset-indigo-600">
